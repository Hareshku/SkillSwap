...
const Login = () => {
  ...
  const { login, loginUser, loginAdmin, error, clearError, isAuthenticated, user } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();

  const from = location.state?.from?.pathname || (loginType === 'admin' ? '/admin' : '/discover');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        const redirectPath = result.data.user.role === 'admin' ? '/admin' : '/discover';
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Sign in to your GrowTogather account</p>
        </div>
      </div>
